...
| 4b | ~~**Executable `macro-composite` report**~~ — **BUILDER SHIPPED on `main`** (PR #24). Phase-classifier-only first cut: `build_macro_composite_report(window_id) -> dict` + `render_macro_composite_markdown(report) -> str`, deterministic / offline / fixture-backed, drift-guarded against the diagnostics fixture. CLI / API / Streamlit / manifest / audit / persistence exposure for this report is still deferred; `classify_regime` integration is also deferred to a follow-up slice. | medium | medium | Catalog is now two-executables (crisis-dossier + macro-composite). The deferred exposure slices can layer over the generic manifest / audit / persistence surfaces without modifying the builder. |
...
- **Pre-merge gate:** full suite green (currently at 865 on `main`, +310 since `v0.8.0`; remaining slices will continue the growth), no `.py` changes outside the allowlist for each slice, CHANGELOG entry present.
